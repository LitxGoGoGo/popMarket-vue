      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cartInfo,index) in cartInfoList" :key="index">
          ...
          <li class="cart-list-con5">
            <a href="javascript:void(0)" class="mins" @click="cartInfo.skuNum>1? cartInfo.skuNum--:cartInfo.skuNum=1">-</a>
            <input autocomplete="off" type="text" :value="`${cartInfo.skuNum}`" minnum="1" class="itxt" >
            <a href="javascript:void(0)" class="plus" @click="cartInfo.skuNum++">+</a>
          </li>
...
export default {
  ...
  computed: {
    ...mapGetters(['cartList']),
    cartInfoList() {
      return this.cartList.cartInfoList || [] //涉及到请求不完全,没拉得到数据的可以返回空的对应结构,以免报错属性undefined
    },
    totalPrice(){
      let sum = 0
